...
const PurchaseModal = ({
    ...
    const handleBulkNFT = async () => {
        ...
        let total_price = 0;
        let total_msg = [];
        for (let i = 0; i < selNFTs.length; i++) {
            const nft = selNFTs[i];
            const contractAddress = nft.collection_key;
            const price = nft.auction.price[0].amount;
            ...
            total_msg.push(nft_msg);
        };

        const fund = Math.ceil(total_price * 1.02);
        ...
        const tx = await signingCosmWasmClient?.execute(senderAddress, CONTRACT_ADDRESS, batch_bids, gas, "", fee);
    ...
    let totalPrice = selNFTs.reduce((sum, nft) => sum + nft.auction.price_float, 0);
            ...
            <Modal.Body className="dark:bg-sysDark text-black dark:text-white rounded-xl rounded-tl-none w-[480px]">
                <div className='relative text-2xl mb-5'>
                    <h1 className='font-MarvinVisions tracking-widest'>
                        {step === 0 ? `Approve Purchase (${selNFTs.length})` : "Your Purchase Is Complete!"}
                    </h1>
                ...
                {/* step 0 start ----- */}
                {
                    ...
                    <>
                        <div className='py-3'>
                            {
                                selNFTs.map(nft => (
                                    <div key={nft.name} className='flex justify-between items-center mb-2'>
                                        <div className='flex items-center gap-2'>
                                            <img src={nft.image} className='w-16 rounded-lg' alt="" />
                                            <div>
                                        ...
                                        <div className='text-center'>
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="">{`${nft.auction.price_float} SEI`}</span>
                                            </div>
                        ...
                        <div className='flex justify-between items-center'>
                            ...
                            <div className='text-center'>
                                <div className="flex items-center gap-1">
                                    ...
                                    <span className="text-xl">{`${totalPrice} SEI`}</span>
                                </div>
                        ...
                        <button onClick={handleBulkNFT} disabled={loading} className='uppercase text-white bg-sysPink rounded rounded-tl-none w-full py-2'>Buy NFTs</button>
                    </>
                }
                {/* step 1 start ------ */}
                {
                    ...
                    <>
                        ...
                        <div className='flex gap-2'>
                            <a href={`https://www.seiscan.app/pacific-1/txs/${txHash}`} target='_blank' className='uppercase text-white text-center bg-opacity10 rounded rounded-tl-none w-full py-2'>View On Seiscan</a>
                            <button onClick={() => setOpenModal(false)} className='uppercase text-white bg-sysPink rounded rounded-tl-none w-full py-2'>Close</button>
                        </div>
                    </>
                }
            </Modal.Body>
